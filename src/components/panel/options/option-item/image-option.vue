<template>
  <el-popover placement="top" width="300">
    <el-form ref="form" :model="optionInfo" label-width="80px" size="mini" :rules="rules">
      ...
      <el-form-item v-if="!item.dynamic" label="图片地址" prop="value">
        <el-input v-model="optionInfo.value" @change="(val) =>{optionInfo.defaultValue = val}"></el-input>
      </el-form-item>
      <el-form-item v-if="item.dynamic" label="变量名称" prop="name">
        <el-input v-model="optionInfo.name" @change="nameChange"></el-input>
      </el-form-item>
      <el-form-item v-if="item.dynamic" label="默认值" prop="defaultValue">
  ...
  data() {
    return {
      ...
      rules: {
        ...
        value: { required: true, trigger: 'change', message: '请输入图片地址' },
