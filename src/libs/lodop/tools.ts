...
import {TempItem,Temp,License,TempTabelCol,TempItemStyle} from '@/types/index' 
/**
 * 通过模板和模板数据生成打印内容
 * @param temp 打印模板
 * @param data 模板数据
 ...
 * @param data 表格数据
 */
export const tableTempTohtml = (columns:TempTabelCol[], data:any[], style:TempItemStyle) => {
 ...
 * @param value 图片相对地址
 */
export const imageTempTohtml = (value:string) => {
 ...
 * @param value 模板值
 */
export const strTempToValue = (str:string, value:string) => {
  let reg = /({[^}^{]*})/g
  return str.replace(reg, value)
}
export const htmlTempTohtml = (val:string, style:TempItemStyle) => {
  // 表格全局样式
  let styleStr = 'text-align:' + style.Alignment + ';'
  styleStr += 'font-size:' + style.FontSize + 'pt;'
  styleStr += 'color:' + style.FontColor + ';'
